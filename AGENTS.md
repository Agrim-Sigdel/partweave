...
- Keep the published tarball lean — only `dist/`, `modules/`, and `README`/`LICENSE`/`NOTICE`
...
- `packages/cli/dist/` is stale until you `build` — always rebuild before scaffolding to test
  a source change.
- `bundle-modules.mjs` copies `modules/` + `README`/`LICENSE`/`NOTICE` into `packages/cli/` at
